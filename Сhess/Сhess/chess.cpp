...
bool Board::doMove() {
	...
	while (!stop)
	{
		(turn == WHITE) ? cout << "Ход белых" << endl : cout << "Ход чёрных" << endl;
		cout << "Введите координаты (X,Y) фигуры которой хотите сходить и координаты (X,Y) куда хотите сходить. Без пробелов!" << endl;
		cin >> move;
		x1 = move[0] - 48;
		y1 = move[1] - 48;
		x2 = move[2] - 48;
		...
		if (getSquare(x1, y1)->getColor() == turn)
		{
			...
			{
				cout << "Ход невозможен. Попробуйте снова" << endl;
			}
			else
				stop = true;
		}
		else
			cout << "Это не ваша фигура. Попробуйте снова." << endl;
	}
...
bool Board::moveRook(Square* thisRook, Square* thatSpace)
{
	...
	{
		std::cout << "Такой ход Ладьёй невозможен.";
		return false;
	}
}
...
bool Board::makeMove(int x1, int y1, int x2, int y2) {
	...
	{
		std::cout << "За пределами поля" << std::endl;
		return false;
	}
	...
	{
		std::cout << "Нельзя переместиться на свою фигуру" << std::endl;
		return false;
	}
	...
	case PAWN: return movePawn(src, dest);
		break;
	case EMPTY: std::cout << "Здесь нет фигуры" << std::endl;  return false;
		break;
